...
5. Computing stability strenght and accuracy edgeweights
6. Figures stability strenght and accuracy edgeweights
7. Comparing subgroups with NCT
8. Similarity between subgroups with correlations between edgelists
...
First the synthetic data is uploaded. This data is made with the Synthpop library. Since this data is synthetic, the results in this script can be different from the actual results in the paper. Also, because of this, the exact number of patients per subgroup, can differ from the paper.
...
#variabele used to define subgroups later on.
...
```{r}
set.seed(12345)
network_preop <- estimateNetwork(df_preop[, c(26:46)], default = "EBICglasso", tuning = 0.25, threshold = FALSE, corMethod ="spearman")
...
network_no_ftg <- estimateNetwork(df_non_fatigued[, c(30:46)], default = "EBICglasso", tuning = 0.25, threshold = FALSE, corMethod = "spearman")
...
# 4. Figures networks and centrality #########################
```{r}
#layout setup networks 1A, 1B, 2A, 2B
labels_12 <- c("CIS-F", "CIS-C", "CIS-M", "CIS-A", "MOS-Cog", "CES-D", "SF-PhF","SF-ScF","SF-RLP","SF-RLM", "SF-MH","SF-BP","SF-HP","SF-CiH","BN-FU", "BN-VD", "BN-MD", "BN-CD", "BN-HA", "BN-Sz", "BN-Dr")
...
```{r}
#layout setup networks 3A, 3B
labels_3 <- c("MOS-Cog", "CES-D", "SF-PhF","SF-ScF","SF-RLP","SF-RLM", "SF-MH","SF-BP","SF-HP","SF-CiH","BN-FU", "BN-VD", "BN-MD", "BN-CD", "BN-HA", "BN-Sz", "BN-Dr")
groups_3 <- c(rep("MOS-Cog", 1), rep("CES-D", 1),  rep("SF-36", 8), rep("BN-20", 7))
colors_3<- c("#0868ac", "#f0f9e8", "#43a2ca", "#7bccc4")
Layout_fatigue <- averageLayout(network_no_ftg, network_ftg)
...
### Edgeweights
@maxine, eval=false, so it does not run if you run the whole file
...
# 8. similarity: correlation between edgelists #########################
