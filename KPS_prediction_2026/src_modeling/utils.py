...
def train_and_evaluate_scale(X, y, cv_outer, optimizer, feature_set:str, scale:bool=False):
    import time
    import numpy as np
    from sklearn.pipeline import Pipeline
    from sklearn.preprocessing import StandardScaler
    ...
    if scale:
        preprocessor = ColumnTransformer([
            ('binary', Pipeline([
                ('imputer', imputer_binary)
            ]), binary_cols),
            ('continuous', Pipeline([
                ('imputer', imputer_cont),
                ('scaler', scaler)
            ]), continuous_cols)
        ])
    else:
    ...
    # Wrap the optimizer in a pipeline to include preprocessing
    from sklearn.base import clone
    optimizer_pipeline = Pipeline([
        ('preprocess', preprocessor),
        ('model', clone(optimizer))
    ])
    ...
    for train_idx, test_idx in cv_outer.split(X, y):
        ...
        start = time.time()
        optimizer_pipeline.fit(X_train, y_train)
        elapsed = time.time() - start

        # Retrieve best estimator from inner RandomizedSearchCV
        best_model = optimizer_pipeline.named_steps['model'].best_estimator_

        # Predict on test set
        X_test_transformed = optimizer_pipeline.named_steps['preprocess'].transform(X_test)
        y_pred = best_model.predict(X_test_transformed)
        y_pred_proba = best_model.predict_proba(X_test_transformed)[:, 1]

        results_list.append({
            "best_model": best_model,
            "best_score": optimizer_pipeline.named_steps['model'].best_score_,
            "y_true": y_test.values,
            "y_pred": y_pred,
            "y_pred_proba": y_pred_proba,
            "fit_time": elapsed,
...
def run_cox_cv(df, binary_features=None, continuous_features=None, extra_features=None, n_splits=5, random_state=42):
    """
    Run K-fold CV for CoxPHFitter with fold-wise imputation to avoid data leakage,
    and perform proportional hazards (PH) test for each fold.
    Supports optional extra features (e.g., time interactions).
    
    Returns:
        - list of fold C-indices
        - fitted CoxPHFitter models per fold
        - mean C-index
        - DataFrame of fold-wise imputed features
        - dict of fold-wise survival function predictions
        - dict of fold-wise PH test results (Schoenfeld residuals)
    """
    from sklearn.impute import SimpleImputer
    from sklearn.compose import ColumnTransformer
    ...
    all_features = binary_features + continuous_features + extra_features

    kf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_state)
    c_indices = []
    models = []

    # To store fold-wise imputed features, survival predictions, and PH tests
    fold_features = pd.DataFrame(index=df.index, columns=all_features + ['event', 'time_to_event'])
    fold_surv_pred = {}
    fold_ph_test = {}

    for fold, (train_idx, test_idx) in enumerate(kf.split(df, df['event'])):
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ("bin", SimpleImputer(strategy="most_frequent"), binary_features),
                ("cont", SimpleImputer(strategy="median"), continuous_features + extra_features)
            ]
        )

        train_imputed = pd.DataFrame(
            preprocessor.fit_transform(train_df[all_features]),
            columns=all_features,
            index=train_df.index
        )
        test_imputed = pd.DataFrame(
            preprocessor.transform(test_df[all_features]),
            columns=all_features,
            index=test_df.index
        )

        train_df[all_features] = train_imputed
        test_df[all_features] = test_imputed

        assert not train_df[all_features].isna().any().any(), "NaNs remain in training features"
        assert not test_df[all_features].isna().any().any(), "NaNs remain in test features"

        # Fit Cox model
        cph = CoxPHFitter()
        cph.fit(train_df, duration_col='time_to_event', event_col='event', show_progress=False)
        test_pred = cph.predict_partial_hazard(test_df)
        ci = concordance_index(test_df['time_to_event'], -test_pred, event_observed=test_df['event'])
        c_indices.append(ci)
        models.append(cph)

        # Store imputed features and original target columns
        fold_features.loc[test_df.index, all_features] = test_df[all_features]
        fold_features.loc[test_df.index, ['event', 'time_to_event']] = test_df[['event', 'time_to_event']]

        # Define timeline in months (for example, from 0 to max follow-up)
        timeline = np.arange(0, df['time_to_event'].max() + 1)  # monthly intervals

        # Store fold-wise survival predictions
        fold_surv_pred[fold] = cph.predict_survival_function(test_df[all_features], times = timeline)

        # PH test on training data for this fold
        ph_test_results = proportional_hazard_test(cph, train_df, time_transform='rank')
        fold_ph_test[fold] = ph_test_results.summary
        
        # plot the schoenfeld residuals of those that violate the ph assumption
        if any(ph_test_results.summary['p'] < 0.05):
            violating_vars = ph_test_results.summary.index[ph_test_results.summary['p'] < 0.05].tolist()
            print(f"Fold {fold}: PH violations detected in {violating_vars}")
            for var in violating_vars:
                cph.check_assumptions(train_df, p_value_threshold=0.05, show_plots=True, columns=[var])

    mean_ci = np.mean(c_indices)
    return c_indices, models, mean_ci, fold_features, fold_surv_pred, fold_ph_test


def run_cox_timevarying_cv(df, binary_features=None, continuous_features=None, n_splits=5, random_state=42):
    """
    Fold-wise CV using CoxTimeVaryingFitter for variables violating PH assumption.
    Automatically detects PH violations and converts them to time-varying covariates.
    
    Returns:
        - c_indices: list of fold-wise concordance indices
        - models: list of fitted CoxTimeVaryingFitter models per fold
        - mean_cindex: mean concordance index
        - fold_features: DataFrame of fold-wise imputed features
        - fold_surv_pred: dict of fold-wise survival function predictions
        - fold_ph_test: dict of fold-wise PH test results from initial CoxPH
    """
    import numpy as np
    import pandas as pd
    from sklearn.impute import SimpleImputer
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(df, df['event'])):
        ...
        # --- Step 1: Fit standard CoxPH to detect PH violations ---
        cph = CoxPHFitter()
        cph.fit(train_df, duration_col='time_to_event', event_col='event', show_progress=False)
        ph_test_results = proportional_hazard_test(cph, train_df, time_transform='rank')
        violating_vars = ph_test_results.summary.index[ph_test_results.summary['p'] < 0.05].tolist()
        ...
        # --- Step 2: Convert violating vars to time-varying start-stop format ---
        if violating_vars:
            print(f"Fold {fold}: PH violations detected in {violating_vars}")
            # For simplicity, create a single interval per individual with time interaction
            tv_train = train_df.copy()
            tv_test = test_df.copy()
            for var in violating_vars:
                tv_train[var + '_tv'] = tv_train[var] * np.log(tv_train['time_to_event'] + 1e-8)
                tv_test[var + '_tv'] = tv_test[var] * np.log(tv_test['time_to_event'] + 1e-8)

            tv_features = all_features + [v + '_tv' for v in violating_vars]
            # CoxTimeVaryingFitter expects columns: id, start, stop, event + covariates
            tv_train_long = tv_train.copy()
            tv_train_long['start'] = 0
            tv_train_long['stop'] = tv_train_long['time_to_event']
            tv_train_long['stop'] += 1
            tv_train_long['event'] = tv_train_long['event']

            tv_test_long = tv_test.copy()
            tv_test_long['start'] = 0
            tv_test_long['stop'] = tv_test_long['time_to_event']
            tv_test_long['stop'] += 1
            tv_test_long['event'] = tv_test_long['event']

            ctv = CoxTimeVaryingFitter()
            ctv.fit(tv_train_long[['start', 'stop', 'event'] + tv_features],
                    id_col=None, start_col='start', stop_col='stop', event_col='event', show_progress=False)
            test_pred = ctv.predict_partial_hazard(tv_test_long[tv_features])
            models.append(ctv)

        else:
            # No violations: keep standard CoxPH
            test_pred = cph.predict_partial_hazard(test_df[all_features])
            models.append(cph)
        ...
        # Store fold-wise features and survival predictions
        fold_features.loc[test_df.index, all_features] = test_df[all_features]
        fold_features.loc[test_df.index, ['event', 'time_to_event']] = test_df[['event', 'time_to_event']]
        fold_surv_pred[fold] = test_pred

    mean_cindex = np.mean(c_indices)
    return c_indices, models, mean_cindex, fold_features, fold_surv_pred, fold_ph_test