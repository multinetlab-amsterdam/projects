...
The Cx created matrices are then visualized. 
...
Computation of GH comparisons between matrices.
The Dx matrices (created with the algorithm "from_cx_to_dx") are compared using the Gromov-Hausdorff distance, 
we create a GH matrix where all comparisons between the Dx matrices  are stored and we visualize the GH matrix using the seaborn library. 

In the code is explained how to perform this analysis. We first need to create an object containing all the matrices we want to compare: 
#Code to create a big object with all the Dx matices to then create GH matrix
txt_files_dx = glob.glob('Dx_matrix_YOUR_FOLDER/dx*') # HERE YOU change with the name of the folder where you have the Dx matrices. If the name of the matrices are dx_1, dx_2 using dx* you upload all of them.
