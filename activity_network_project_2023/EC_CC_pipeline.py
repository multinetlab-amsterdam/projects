...
def calculate_network_metrics(PLI_dir, output_dir, freq_dict, HC=False):
    ...
    for key, value in freq_dict.items():
        ...
        for d in [0.1, 0.2, 0.3]: # could be flexible to other densities...
            ...
            # --- Step 4.) Save df ---#
            df_all_density.to_csv(
                output_dir + key + "/desired_file_name_" + str(d) + ".csv"
            )
...
def standardize_metrics(
    input_dir, input_HC_dir, output_dir, freq_dic, HC_subs_list, HC=False
):
    """
    ...
    for key, freq in freq_dict.items():
        ...
        for d in [0.1, 0.2, 0.3]:
            ...
            df_HC_CC.rename(
                columns={"CC": "CC_mean_HC", "std": "CC_std_HC"}, inplace=True
            )
            ...
            df_HC_EC.rename(
                columns={"EC": "EC_mean_HC", "std": "EC_std_HC"}, inplace=True
            )

            # --- Step 2.) add means and std repeatedly to rows (i.e. rows) of subs dataframe to allow for easy standardization later--- #

            ### Clustering Coefficient ###
            subs_df["CC_mean_HC"] = subs_df["rois"].apply(
                lambda x: df_HC_CC["CC_mean_HC"].loc[x]
            )
            subs_df["CC_std_HC"] = subs_df["rois"].apply(
                lambda x: df_HC_CC["CC_std_HC"].loc[x]
            )

            ### Eigenvector centrality ###
            subs_df["EC_mean_HC"] = subs_df["rois"].apply(
                lambda x: df_HC_EC["EC_mean_HC"].loc[x]
            )
            subs_df["EC_std_HC"] = subs_df["rois"].apply(
                lambda x: df_HC_EC["EC_std_HC"].loc[x]
            )

            # --- Step 3.) Standardize CC and EC and add as new columns to dataframe --- #

            ### Clustering Coefficient ###
            subs_df["CC_z"] = (subs_df["CC"] - subs_df["CC_mean_HC"]) / subs_df[
                "CC_std_HC"
            ]

            ### Eigenvector centrality ###
            subs_df["EC_z"] = (subs_df["EC"] - subs_df["EC_mean_HC"]) / subs_df[
                "EC_std_HC"
            ]
...
standardize_metrics(
    input_dir, input_HC_dir, output_dir, freq_dict, HC_subs_li
)  # standardize patients metrics
standardize_metrics(
    input_HC_dir, input_HC_dir, output_dir_HC, freq_dict, HC_subs_li, HC=True
