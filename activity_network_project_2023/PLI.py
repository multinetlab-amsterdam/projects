    ...
    for index, row in pd.read_csv(subjects).iterrows():
        ...
        if len(files_list) == 0:
            ...
            for epoch_file, name in zip(range(len(files_list)),files_list):
                ...
                timeseries = pd.read_csv(files_list[epoch_file], index_col=False,
                ...
                for i in range(0,3):
                    ...
                    filt_piece = fft_filt(epoch_piece, freq_value[0], freq_value[1], fs=fs, nr_rois = nr_rois)[0]
...
freq_dict = {'BB': [0.5, 48], 'delta': [0.5, 4],'theta':[4, 8],'lower_alpha':[8, 10]}
...
for freq, freq_value in tqdm(freq_dict.items()):
    ...
    nr_rois = np.arange(210)#(10,) if only run for roi 11 # note that python indexes at 0!
    ...
    if save_output == True:
        ...
        for index, row in subjects.iterrows():
                ...
                if (all_pli_matrices.shape[0] > 1): 
                    ...
                    #save the pli
                    df_pli.to_csv(path_or_buf=dir_output_pli
                                    + row['Case_ID'] + '_'
                                    + str(row['MM']) + '_'
                                    + str(row['Atlas']) + '_'
