...
library(MASS)
library(MuMIn)
library(lmerTest) #vb deze library toevoegen
```

# 1. Synthesizing mock datasets
The analyses in the 2022 fatigue paper consist of the Glioma Cohort and a subgroup, the Longitudinal subgroup. Here we provide mock data to run the analyses with. The data does not reflect the real distrubution #vb spelfout of the actual data, therefor#vb spelfout the results produced in this script should merely be used to understand the analyses itself.
...
df <- data.frame (random_id = c(1:222, 1:111),
                   ...
                   phase_disease  = c(sample(c("Baseline", "Treatment", "Stable", "Progression"), 333, replace=TRUE)),
...
#vb geen postop patienten nodig in longitudinal sample?
```

# 2. percentages of fatigued patients #vb ook hoofdletter begin zin?
...
# Treatment
treatment <- df %>% dplyr::filter(phase_disease== "Treatment")
...
# Stable disease
stable <- df %>% dplyr::filter(phase_disease== "Stable")
...
# Progression
progression <- df %>% dplyr::filter(phase_disease== "Progression")
...
```{r}
df_1x1 <- data_1x1(
  array_1 = df_preok$CIS_fatigue, #change variabel #vb variable in english
  array_2 = df_postok$CIS_fatigue, #change variabel  #vb variable in english
  jit_distance = .09,
  jit_seed = 321)
```
...
raincloud_figure <- raincloud_1x1_repmes(
  ...
  xlab(NULL)+
  ylab("CIS total score") +
  theme_classic(base_size = 18)

raincloud_figure

#vb also change y axis with different variables
```

# 4. linear mixed model Glioma Cohort #vb hoofdletter begin zin
...
```{r, message=FALSE}
vars = c("SF36_PF", "SF36_SC", "SF36_RLP", "SF36_RLM", "SF36_MH", "SF36_P", "SF36_HP", "SF36_CIH", "CESD_total", "mos_cog",  "BN20_FU", "BN20_MD", "BN20_S", "side", "kps", "sex", "age", "verhage", "idh", "location", "assessment", "histology", "dexa" , "phase_disease", "ct_before_assessment", "ct_before_assessment")
...
summary(lmm_individual_variable[[4]])  #change the number to see the other variabels. Number of myvars=26.
```

See table 2 in manuscript. Variabels to exclude for the next step: age, dexa, location, idh, histology, phase_disease, ct_before_assessment, ct_before_assessment, grade assessment

#vb consider excluding the variables based on your created data to check if you excluded the correct variables according to your methods --> maybe not possible I checked 8, all with a high p-value.  
...
# 7. multiple linear regression #vb hoofdletter
